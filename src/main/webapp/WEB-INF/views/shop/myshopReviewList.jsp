...
// 				댓글보여주는거
			...
			//댓글이 없을 때
				...
				if(data.shopMemberId == data.loginMemberId){					
					...
					html += "<button type='button' id='BtnGoWrite' onclick='reply_review(commentText"+i+","+ data.reviewList[i].reviewNo +")' class='btn'>댓글쓰기</button>";
					html += "</td></tr>";				
				}
			}
			
			html += "<tr style='display:none' id='commentText"+i+"'><td class='tg-0lax' colspan='4'> <input type='text' id='updateContent"+i+"' class='updateContent'><button type='button' id='BtnGoUpdate' onclick='update_review("+ data.reviewList[i].reviewNo +","+i+")' class='btn'>댓글등록</button></td></tr>";
